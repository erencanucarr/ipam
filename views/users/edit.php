...
<head>
    <meta charset="UTF-8">
    <title>Edit User - Simple IPAM</title>
    ...
    <div class="container">
        ...
        <form method="post" action="index.php?page=users&action=edit&id=<?= $user['id'] ?>">
            <div class="form-group">
                ...
                <input id="name" type="text" name="name" value="<?= htmlspecialchars($user['name']) ?>" required autofocus>
            </div>
            <div class="form-group">
            ...
            <div class="form-group">
                ...
                <select id="role" name="role" required>
                    ...
                    <option value="support" <?= $user['role'] === 'support' ? 'selected' : '' ?>>Support (Edit IPs Only)</option>
                    <option value="admin" <?= $user['role'] === 'admin' ? 'selected' : '' ?>>Admin (Full Access)</option>
                </select>
            </div>
            <button type="submit">Update User</button>
        </form>
        <a class="back-link" href="index.php?page=users">Back to User List</a>
    </div>
</body>
</html>